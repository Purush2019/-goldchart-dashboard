"""
Gold Chart — Standalone TradingView-Style App (Coinbase Data)
================================================================
...
Displays a beautiful live chart in your browser.

NO API key needed — uses public endpoints.
NO Plus500. NO Playwright. NO paid subscriptions.
Just run:  python gold_chart.py
...
def _utc_to_est(utc_ts):
    """Shift a UTC unix timestamp so lightweight-charts displays it as EST."""
    dt_utc = datetime.fromtimestamp(utc_ts, tz=timezone.utc)
    est_offset = dt_utc.astimezone(_NY_TZ).utcoffset().total_seconds()
    return int(utc_ts + est_offset)
...
# =============================================================================
# KEEP-ALIVE — Prevent sleep, lock, display off
# =============================================================================
...
def keep_alive_loop():
    """Background thread: jiggle mouse + SetThreadExecutionState every 30s."""
    if not IS_WINDOWS:
        return
    while True:
        try:
            ...
            # tiny invisible mouse jiggle to reset idle timer
...
def start_keep_alive():
    """Start keep-alive in a daemon thread (Windows only)."""
    if not IS_WINDOWS:
        print("   [OK] Linux server — keep-alive not needed (no sleep/lock)")
        return None
...
REFRESH_SEC = 1            # Refresh every 1 second (fast mode)
HISTORY_HOURS = 72         # How many hours of history to fetch (3 days)

# Port (HTTP + WebSocket on single port for easy mobile/tunnel access)
HTTP_PORT = 8080
WS_PORT = HTTP_PORT  # same port — consolidated
...
def compute_all(opens, highs, lows, closes):
    """Compute all indicators and generate signals for the full dataset."""
    ...
    # Generate signals — Pine Script strategy.entry / strategy.close logic
    # In Pine Script:
    #   strategy.entry("Long")  → opens long, auto-closes any short
    #   strategy.entry("Short") → opens short, auto-closes any long
    #   strategy.close("Long")  → exits long to flat (ONLY from SSL3 exit)
    #   strategy.close("Short") → exits short to flat (ONLY from SSL3 exit)
    # Entries are processed BEFORE exits. Last entry wins if both fire.
    ...
    for i in range(1, n):
        ...
        # ── ENTRY signals (SSL baseline, EMA crossover, UT Bot) ──
        long_entry  = ssl_buy or ema_buy or ut_buy
        short_entry = ssl_sell or ema_sell or ut_sell

        # ── EXIT signals (ONLY from SSL3 exit channel) ──
        long_exit  = ssl3_sell
        short_exit = ssl3_buy
        ...
        # 1) Process entries first. Short entry has priority (last in Pine Script = wins)
        #    strategy.entry auto-closes the opposite position
        ...
        # 2) Process exits ONLY if no entry action was taken
        #    strategy.close only fires from SSL3 exit channel
        if action is None:
        ...
        # Record marker — TradingView style labels
        if action:
            if action == "open_long":
...
def build_chart_message(raw_candles):
    ...
    # Convert timestamps from UTC -> EST for display
    est_timestamps = np.array([_utc_to_est(int(t)) for t in timestamps])

    # Build candle data
    candles = []
    for i in range(n):
        candles.append({
            "time": int(est_timestamps[i]),
    ...
    # Build indicator line data
    def make_line(arr):
        line = []
        for j in range(len(arr)):
            if not np.isnan(arr[j]):
                line.append({"time": int(est_timestamps[j]), "value": round(float(arr[j]), 2)})
        return line

    # Build markers — TradingView style labels and colors
    chart_markers = []
    for m in indicators["markers"]:
        i = m["idx"]
        t = int(est_timestamps[i])
        a = m["action"]
        if a == "open_long":
...
async def ws_handler(websocket):
    """Handle WebSocket connections — send chart data on connect."""
...
async def process_request(connection, request):
    ...
    Serves chart.html for regular HTTP; returns None for WS upgrades.
    """
    from websockets.http11 import Response
    from websockets.datastructures import Headers
    ...
    path = request.path
    if path == "/" or path == "/chart.html":
        return _serve_file(os.path.join(chart_dir, "chart.html"), "text/html; charset=utf-8")
    if path == "/qr" or path == "/qr.html":
        return _serve_file(os.path.join(chart_dir, "qr.html"), "text/html; charset=utf-8")
    if path == "/favicon.ico":
        return Response(204, "No Content", Headers(), b"")
    # Config endpoint — returns server info (for mobile/ngrok)
    if path == "/config.json":
        import socket
        local_ip = "127.0.0.1"
        try:
...
# =============================================================================
# PORT CLEANUP — kill anything holding our ports before starting
# =============================================================================
...
def kill_port(port):
    ...
    try:
        if IS_WINDOWS:
            ...
            # Linux: use fuser to kill processes on the port
...
async def main():
    ...
    print(f"""
    ╔═══════════════════════════════════════════════════════════╗
    ║       GOLD CHART — COINBASE Derivatives                   ║
    ║                                                           ║
    ║  Real Gold Futures from COINBASE public API               ║
    ...
    ║    • Bollinger Bands (20, 2.0)                           ║
    ║                                                           ║
    ║  ✓ AUTO-RESTART enabled — server never stays down        ║
    ║  ✓ KEEP-ALIVE enabled — system won't sleep or lock       ║
    ║                                                           ║
    ...
    # 0. Keep-alive (prevent sleep/lock)
    start_keep_alive()
    ...
    ws_server = await websockets.serve(
        ...
        max_size=10 * 1024 * 1024,  # 10 MB — chart data with 3000+ candles is large
    )
    ...
    try:
        ...
        _local_ip = "127.0.0.1"
    print(f"   ✓ Server on http://0.0.0.0:{HTTP_PORT}/chart.html")
    print(f"   📱 Mobile (same WiFi): http://{_local_ip}:{HTTP_PORT}/chart.html")

    # 3. Download data and compute indicators
    ...
    if not os.environ.get("DASHBOARD_MODE"):
        webbrowser.open(f"http://localhost:{HTTP_PORT}/chart.html")
...
def run_with_watchdog():
    """Watchdog: restart main() automatically if it crashes."""
    MAX_FAST_CRASHES = 5          # if it crashes 5 times within 60s, pause longer
    FAST_CRASH_WINDOW = 60        # seconds
    crash_times = []
    attempt = 0
    ...
    while True:
        ...
        try:
            ...
            # Free ports in case previous instance left them occupied
            free_ports()
            ...
            if len(crash_times) >= MAX_FAST_CRASHES:
                wait = 30
                print(f"   ⚠️  {MAX_FAST_CRASHES} crashes in {FAST_CRASH_WINDOW}s — waiting {wait}s before retry...")
                crash_times.clear()
            elif elapsed < 5:
                wait = 5
